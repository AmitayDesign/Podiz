...
exports.updateEpisodeDaily = async () => {
  ...
  let number = 500;
  let showOffset = 0;
  let docsSnap = await admin.firestore().collection("shows").orderBy("publisher").startAt(showOffset).limit(number).get();
  // let new_episodes = 0;
  while(docsSnap.size != 0) {
    docsSnap.forEach(async (d) => {
      ...
      let showJson = await helpers.fetchFromHost("/shows/" + doc_id + "?market=US", accessToken);
      let show = await showJson.json();
      let show_name = show["name"];
      console.log(show);
      console.log(show["episodes"]["total"]);
      
      let size = await helpers.getNumberOfEpisodes(doc_id);
      console.log(size);
    ...
    showOffset += 500;
    docsSnap = await admin.firestore().collection("shows").orderBy("publisher").startAt(showOffset).limit(number).get();    
  }
};
